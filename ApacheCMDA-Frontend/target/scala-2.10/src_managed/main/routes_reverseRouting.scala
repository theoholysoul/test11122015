...
// @LINE:39
def workflows(): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "workflow/workflows")
}
                                                

// @LINE:38
def home(email:String = null, vfile:String = null, dataset:String = null): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "workflow" + queryString(List(if(email == null) None else Some(implicitly[QueryStringBindable[String]].unbind("email", email)), if(vfile == null) None else Some(implicitly[QueryStringBindable[String]].unbind("vfile", vfile)), if(dataset == null) None else Some(implicitly[QueryStringBindable[String]].unbind("dataset", dataset)))))
}
...
def workflows : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      function() {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "workflow/workflows"})
      }
   """
)
...
// @LINE:38
def home : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.WorkflowController.home",
   """
      function(email,vfile,dataset) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "workflow" + _qS([(email == null ? null : (""" + implicitly[QueryStringBindable[String]].javascriptUnbind + """)("email", email)), (vfile == null ? null : (""" + implicitly[QueryStringBindable[String]].javascriptUnbind + """)("vfile", vfile)), (dataset == null ? null : (""" + implicitly[QueryStringBindable[String]].javascriptUnbind + """)("dataset", dataset))])})
      }
   """
)
...
def workflows(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.WorkflowController.workflows(), HandlerDef(this, "controllers.WorkflowController", "workflows", Seq(), "GET", """""", _prefix + """workflow/workflows""")
)
                      

// @LINE:38
def home(email:String, vfile:String, dataset:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.WorkflowController.home(email, vfile, dataset), HandlerDef(this, "controllers.WorkflowController", "home", Seq(classOf[String], classOf[String], classOf[String]), "GET", """ Workflow""", _prefix + """workflow""")
)
